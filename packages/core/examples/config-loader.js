...
export const loadConfig = async () => {
    ...
    if (process.env.LLM_PROVIDER)
        config.models.apiProvider = process.env.LLM_PROVIDER;
    if (process.env.LLM_LIGHT_MODEL)
        config.models.light = process.env.LLM_LIGHT_MODEL;
    if (process.env.LLM_HEAVY_MODEL)
        config.models.heavy = process.env.LLM_HEAVY_MODEL;
    if (process.env.EXECUTION_MODE)
        config.execution = process.env.EXECUTION_MODE;
    if (process.env.MAX_TURNS)
        config.maxTurns = parseInt(process.env.MAX_TURNS, 10);
    // Resolve proper API keys based on provider
    const provider = config.models.apiProvider;
    if (provider === 'google')
        config.models.apiKey = process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY;
    if (provider === 'openai')
        config.models.apiKey = process.env.OPENAI_API_KEY;
    if (provider === 'anthropic')
        config.models.apiKey = process.env.ANTHROPIC_API_KEY;
    if (provider === 'vertex') {
